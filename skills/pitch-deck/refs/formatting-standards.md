...
**AUCTUS branding colors:** `#1F4E79` (dark blue header), `#BDD7EE` (medium blue secondary). Replace Anthropic's example orange/orange-dark colors with these in all patterns.
...
2. **Header row**:
   - Bold text
   - Shaded background (AUCTUS: `#1F4E79` for primary headers)
   - Contrasting text color for readability (white `#FFFFFF` on dark blue)
...
When displaying key metrics (e.g., EV, IRR, MOIC, EBITDA), consider showing relationships between values rather than listing them statically:
...
1. **Colors**: Use the template's brand colors (AUCTUS: `#1F4E79` and `#BDD7EE`)
