...
class TestRunLBOIntegration:
    def test_full_pipeline_produces_three_outputs(
    ...
    def test_sensitivity_no_nan_central_cells(
        self, lbo_assumptions: EntryAssumptions, tmp_path: Path
    ) -> None:
        out_dir = tmp_path / "lbo_models"
        result = run_lbo(lbo_assumptions, out_dir)
        entry_keys = sorted(result.sensitivity_irr.keys(), key=lambda k: float(k[:-1]))
        exit_keys = sorted(next(iter(result.sensitivity_irr.values())).keys(), key=lambda k: float(k[:-1]))
        # Central 3×3 of the 5×5 grid must be finite
        for ek in entry_keys[1:-1]:
            for xk in exit_keys[1:-1]:
                val = result.sensitivity_irr[ek][xk]
                assert math.isfinite(val), (
                    f"NaN in sensitivity grid cell ({ek}, {xk}) — computation error"
                )
