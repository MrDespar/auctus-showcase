...
version: "2.1.0"
description: >
  Renders a fully formatted 15-slide PowerPoint investment pitch deck (.pptx) from
  deterministic model artifacts using python-pptx. Implements Anthropic's full
  template-population workflow (5 phases) with critical anti-patterns, validation loop,
  and MUST requirements. All financial figures sourced exclusively from script outputs —
  no LLM-computed numbers. AUCTUS branding: #1F4E79 dark blue, #BDD7EE medium blue.
triggers:
  - "generate pitch deck"
  - "build pitch deck"
  - "create slide deck"
  ...
  - "fill in the deck"
refs:
  formatting_standards: "skills/pitch-deck/refs/formatting-standards.md"
  slide_templates: "skills/pitch-deck/refs/slide-templates.md"
  xml_reference: "skills/pitch-deck/refs/xml-reference.md"
  calculation_standards: "skills/pitch-deck/refs/calculation-standards.md"
  auctus_criteria: "config/auctus_criteria.yaml"

  - "outputs/pitch_decks/pitch_{slug}_{YYYYMMDD_HHMMSS}.pptx"
---
...
**Read all reference files at task start before beginning any work.** They contain
critical patterns and anti-patterns that will affect your approach.

| File | Purpose |
|------|---------|
| [formatting-standards.md](refs/formatting-standards.md) | Text, bullets, tables, charts, alignment |
| [slide-templates.md](refs/slide-templates.md) | Content mapping guidance for common slide types |
| [xml-reference.md](refs/xml-reference.md) | PowerPoint XML patterns for tables, shapes, arrows |
| [calculation-standards.md](refs/calculation-standards.md) | Financial formulas for verification (CAGR, multiples) |

### Prerequisites

All valuation and modeling workflows should be complete before generating the deck:
...
- `/relative-valuation` → provides Comps and Precedents JSON
- `/3-statement-model` → provides Historical & Projected financials JSON
- `/competitor-analysis` or `/screen-deal` → provides target matrix (AUCTUS scoring, sector context)

If any of these model artifacts are missing, do NOT halt and ask the user for them or run in partial mode. Instead, you MUST proactively execute the missing upstream skills (`/lbo-modeling`, `/dcf-valuation`, `/relative-valuation`, `/3-statement-model`, `/competitor-analysis`) to generate the required dependencies before drafting the deck.

### Data Source Hierarchy

- **All financial figures MUST come from Python script outputs** (LBO JSON, DCF JSON)
- **Never compute financial figures in LLM context** — not even rough estimates
- If a required figure is missing from model outputs, state "Run [workflow] first"

### Execution Environment

**python-pptx only** — AUCTUS runs headless; no Office JS or Office Add-in available.

Check exit code. Non-zero = stop, report error verbatim to user.

### AUCTUS Branding

| Element | Color |
|---------|-------|
| Primary brand | #1F4E79 (dark blue) |
| Secondary / header | #BDD7EE (medium blue) |
| Body text | #000000 (black on white) |
| Header text | #FFFFFF (white on #1F4E79) |

**Validation**: LibreOffice (`soffice --headless`) is available for structural validation.
Always deliver with the disclaimer: "Validated in LibreOffice. Review in PowerPoint before distribution."

### AUCTUS-Specific Rules

**Exactly 15 slides** — do not add or remove slides.
All financial values in EUR millions (€m). IRR `0.0%`. MOIC `0.0×`. EV `€m`.
No pipe/tab-separated tables — all tables must be actual table objects.
No placeholder formatting retained in final output.

### Quality Gate

Run: `pytest tests/test_deck_builder.py -v --tb=short`
Must exit 0. Deck must have exactly 15 slides. Output `.pptx` non-empty.

### Quality Rubric

For each new capability, ensure the following standards are met:
- **Football Field Valuation**: Must display at least 4 valuation methodologies (DCF, LBO, Trading Comps, Precedents) with overlapping bar ranges and a distinct vertical line for the proposed entry valuation.
- **Trading Comparables / Precedents**: Must explicitly show Mean, Median, High, and Low multiples. Outliers must be footnoted.
- **Historical & Projected Financials**: Must bridge actuals to projections seamlessly. Minimum 3 years historical, 5 years projected.
- **Cap Table**: Must correctly compute fully diluted ownership pre and post-transaction.
- **Risk & Mitigants**: Every stated risk must be paired with an explicit, actionable AUCTUS mitigating factor.

### Correct Patterns

When implementing non-obvious slides like the Football Field, use the following code pattern via `python-pptx` to generate aligned floating bars:

```python
# Correct Pattern: Football Field Floating Bar Chart
from pptx.enum.chart import XL_CHART_TYPE
from pptx.util import Inches

# 1. Create a stacked bar chart (not standard bar) to float the ranges
chart_data = CategoryChartData()
chart_data.categories = ['DCF', 'LBO', 'Trading Comps', 'Precedents']
# Series 1 is the 'invisible' base (Low value)
chart_data.add_series('Base', [dcf_min, lbo_min, comps_min, prec_min])
# Series 2 is the 'visible' range (High - Low)
chart_data.add_series('Range', [dcf_max-dcf_min, lbo_max-lbo_min, comps_max-comps_min, prec_max-prec_min])

chart = slide.shapes.add_chart(
    XL_CHART_TYPE.BAR_STACKED, Inches(1), Inches(2), Inches(8), Inches(4), chart_data
).chart

# 2. Make the base series invisible
base_series = chart.series[0]
base_series.format.fill.background() # No fill
base_series.format.line.color.type = None # No line

# 3. Format the range series with AUCTUS brand color
from pptx.dml.color import RGBColor
range_series = chart.series[1]
range_series.format.fill.solid()
range_series.format.fill.fore_color.rgb = RGBColor(0x1F, 0x4E, 0x79) # #1F4E79
```
...
ls -t outputs/target_matrices/*_targets.csv            | head -1
ls -t outputs/valuation_models/*_comps.json            | head -1
ls -t outputs/valuation_models/*_precedents.json       | head -1
ls -t outputs/financials/*_3statement.json             | head -1
```

Record the resolved paths. If critical JSONs are not found, stop and proactively run the missing valuation workflows first.

## STEP 2 — GENERATE DECK

Build the PowerPoint presentation natively in context based on the model artifacts retrieved.

## STEP 3 — QUALITY GATE

Verify:
  - Output `.pptx` file exists and is non-empty
  - Deck has exactly 15 slides

Append completion entry to `logs/agent_activity.log`.

## EXIT CONDITION

Deliver path to the `.pptx` file.
State explicitly: company name, slide count, deal date used.
Include disclaimer: "Validated in LibreOffice. Review in Microsoft PowerPoint before distribution."

## SLIDE MANIFEST (15 SLIDES)

| # | Slide | Key content |
|---|-------|-------------|
| 1 | Cover | Company name, deal date, AUCTUS branding |
| 2 | Executive Summary | EV, MOIC, IRR, entry/exit multiple metric tiles |
| 3 | Business & Market Overview | Revenue, EBITDA, margin, geography, hold period, TAM, tailwinds |
| 4 | Investment Thesis | 6 thesis bullets + AUCTUS score badge |
| 5 | Historical & Projected Financials | 3-statement summary, KPI trends, growth bridge |
| 6 | Trading Comparables | Public peer benchmarking, EV/Rev, EV/EBITDA multiples |
| 7 | Precedent Transactions | M&A transaction comps, deal values, implied multiples |
| 8 | Valuation Summary | Football field chart (DCF, LBO, Comps, Precedents) |
| 9 | DCF Valuation | EV, PV FCFs, terminal value, WACC, TGR |
| 10 | LBO Analysis | Sources & Uses table + Exit Metrics table |
| 11 | Sensitivity Grid | IRR % matrix (entry × exit multiple) |
| 12 | Value Creation Bridge | 4-lever attribution table |
| 13 | Risk Factors & Mitigants | 6 risks with descriptions and specific mitigating factors |
| 14 | Management & Ownership | Key personnel, current cap table, post-deal structure |
| 15 | Appendix | Full LBO model assumptions |

---

# Pitch Deck Authoring (Anthropic Financial Services Reference)
...
| [`slide-templates.md`](refs/slide-templates.md) | Content mapping guidance for common slide types |
| [`xml-reference.md`](refs/xml-reference.md) | PowerPoint XML patterns for tables, shapes, arrows |
| [`calculation-standards.md`](refs/calculation-standards.md) | Financial formulas for verification (CAGR, consensus) |
...
6. Note any calculations that need verification → see [`calculation-standards.md`](refs/calculation-standards.md) for formulas
...
3. Map source data to corresponding template sections → see [`slide-templates.md`](refs/slide-templates.md) for mapping guidance
4. Identify placeholder guidance boxes (colored instruction boxes from task creator)
5. Note any data gaps or mismatches → see [`slide-templates.md`](refs/slide-templates.md#handling-data-template-mismatches) for resolution
...
3. **Then apply formatting** to match template style → see [`formatting-standards.md`](refs/formatting-standards.md)
4. Create tables as actual table objects (NEVER use pipe/tab-separated text) → see [`xml-reference.md`](refs/xml-reference.md#table-implementation)
5. Create arrows/shapes as PowerPoint objects → see [`xml-reference.md`](refs/xml-reference.md#arrow-shapes)
...
| **Actual Table Objects** | Tabular data MUST be table objects, not tab-separated text. See [`xml-reference.md`](refs/xml-reference.md#table-implementation). |
| **Proper Chart/Table Sizing** | Pasted visuals MUST fill designated area. See [`formatting-standards.md`](refs/formatting-standards.md#chart-and-image-handling). |
...
**MUST verify:** After creating any table, verify it is an actual table object. See [`xml-reference.md`](refs/xml-reference.md#critical-verify-tables-are-actual-table-objects) for verification methods.
...
|---------|----------|-----------|
| Unstructured text dumps | Break into bullets (✓, ×, •) | [`formatting-standards.md`](refs/formatting-standards.md#bullet-point-structure) |
| Pipe/tab-separated "tables" | Create actual table objects — text with separators is NOT a table | [`xml-reference.md`](refs/xml-reference.md#table-implementation) |
| Poor text/background contrast | Audit every text element | — |
| Tiny pasted charts | Resize to fill area, paste chart only | [`formatting-standards.md`](refs/formatting-standards.md#proper-sizing-workflow) |
...
| Inconsistent bullets | Define style once, apply to all | [`formatting-standards.md`](refs/formatting-standards.md#bullet-consistency) |
| Inconsistent fonts across boxes | Standardize same-level boxes | [`formatting-standards.md`](refs/formatting-standards.md#font-consistency) |
...
| Text arrows (→, ⟹) | Use PowerPoint shape objects | [`xml-reference.md`](refs/xml-reference.md#arrow-shapes) |
...
For XML implementation, see [`xml-reference.md`](refs/xml-reference.md#table-implementation).
...
For detailed data requirements, formatting principles, and example column headers for each slide type, see [`slide-templates.md`](refs/slide-templates.md#common-slide-types-and-data-requirements).
...
- [ ] Calculated values verified against formulas (see [`calculation-standards.md`](refs/calculation-standards.md))
